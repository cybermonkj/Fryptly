...
// If the user does not have membership "Premium" exclude
if ( !pmpro_hasMembershipLevel( "Basic", $user_id ) ) return true;
return $result;
}
...
function my_gettext_pay_by_check($translated_text, $text, $domain)
{
	if($domain == "pmpro-pay-by-check" && $text == "Pay by Check")
		$translated_text = "Use a Fryptly Redeem code";		//change the text here
...
function my_gettext_membership_to_package( $translated_text, $text, $domain )
{
	if($domain == "pmpro" || $domain == "paid-memberships-pro") {
		...
		$translated_text = str_replace('membership', 'Subscription', $translated_text);
		$translated_text = str_replace('Discount Code', 'Redeem code', $translated_text);
		$translated_text = str_replace('Submit and Check out ', 'Register', $translated_text);
		$translated_text = str_replace('Your payment is currently pending. You will gain access to this page once it is approved.', 'Your subscription has expired or has not been Approved, Please do hold on or goto https://fryptly.com/subscription if you are not redirected automatically', $translated_text);

		//can add additional translations here
  	}
  
	return $translated_text;
}
add_filter('gettext', 'my_gettext_membership_to_package', 10, 3);
