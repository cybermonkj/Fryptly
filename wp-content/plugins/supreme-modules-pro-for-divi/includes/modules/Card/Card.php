...
class DSM_Card extends ET_Builder_Module {
	...
	public function render( $attrs, $content = null, $render_slug ) {
		...
		$image = $image ? sprintf(
			...
			$image_as_background !== 'on' ? $image : '<div class="dsm_card_image_background"></div>',
			esc_attr( $generate_css_image_filters ),
			( $badge_custom_position === 'on' ? $badge_text : '' ),
			( $badge_custom_position === 'off' ? $badge_text : '' ),
			$badge_custom_position === 'on' ? '' : " dsm_card_badge_{$badge_position}",
		...
		if ( $multi_view->has_value( 'title' ) || $multi_view->has_value( 'subtitle' ) || $multi_view->has_value( 'content' ) || $multi_view->has_value( 'button_text' ) ) {
			...
			$content_wrapper = sprintf(
				'<div class="dsm_card_wrapper%5$s">
					%1$s
					%2$s
					%3$s
					%4$s
		...
		$this->add_classname(
			array(
				...
				$image_background_animation !== 'none' ? "dsm_card_image_animation dsm_card_image_animation_${image_background_animation}" : '',
