...
class DSM_Star_Rating extends ET_Builder_Module {
	...
	public function render( $attrs, $content = null, $render_slug ) {
		...
		$stars_output = self::get_starrating( array(
		...
		if ( '' !== $title ) {
			$title = sprintf( '<%1$s class="dsm-star-rating-title et_pb_module_header">%2$s</%1$s>',
		...
		if ('inline-block' === $stars_display_type) {
			if ( '' !== $title_gap_tablet || '' !== $title_gap_phone || '7px' !== $title_gap ) {
				...
				et_pb_generate_responsive_css( $title_gap_values, '%%order_class%% .dsm-star-rating-title', 'margin-right', $render_slug );
			}
		} else {
			if ( '' !== $title_gap_tablet || '' !== $title_gap_phone || '7px' !== $title_gap ) {
				$title_gap_responsive_active = et_pb_get_responsive_status( $title_gap_last_edited );
				...
				et_pb_generate_responsive_css( $title_gap_values, '%%order_class%% .dsm-star-rating-title', 'margin-bottom', $render_slug );
			}
		}
		...
		if ( '' !== $stars_size_tablet || '' !== $stars_size_phone || '14px' !== $stars_size ) {
			...
			et_pb_generate_responsive_css( $stars_size_values, '%%order_class%% .dsm-star-rating', 'font-size', $render_slug );
		}
		...
		if ( '' !== $stars_gap_tablet || '' !== $stars_gap_phone || '' !== $stars_gap ) {
			...
			et_pb_generate_responsive_css( $stars_gap_values, '%%order_class%% .dsm-star-rating i:not(:last-of-type)', 'margin-right', $render_slug );
		}
