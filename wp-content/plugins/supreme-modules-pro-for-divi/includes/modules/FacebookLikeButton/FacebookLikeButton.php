...
class DSM_FacebookLikeButton extends ET_Builder_Module {
	...
	public function get_fields() {
		return array(
			...
			'fb_show_faces'    => array(
				'label'            => esc_html__( 'Show Faces', 'dsm-supreme-modules-pro-for-divi' ),
				'type'             => 'select',
				...
				'toggle_slug'      => 'main_content',
				'description'      => esc_html__( 'Specifies whether to display profile photos below the button (standard layout only).', 'dsm-supreme-modules-pro-for-divi' ),
	...
	public function render( $attrs, $content = null, $render_slug ) {
		...
		$fb_show_faces   = $this->props['fb_show_faces'];
		...
		$output = sprintf(
			'<div class="fb-like dsm-facebook-like" data-href="%1$s" data-layout="%2$s" data-colorscheme="%3$s" data-show-faces="true" data-action="%5$s" data-size="%6$s" data-share="%7$s" data-width="280px"></div>
			',
			esc_url( $fb_url ),
			esc_attr( $fb_layout ),
			esc_attr( $fb_color_scheme ),
			esc_attr( $fb_show_faces ),
			esc_attr( $fb_action ),
			esc_attr( $fb_size ),
			esc_attr( $fb_share )
		);
