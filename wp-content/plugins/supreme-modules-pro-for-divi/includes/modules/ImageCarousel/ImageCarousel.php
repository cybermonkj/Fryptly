...
class DSM_ImageCarousel extends ET_Builder_Module {
	...
	public function render( $attrs, $content = null, $render_slug ) {
		...
		if ( '' !== $arrow_size_tablet || '' !== $arrow_size_phone || '' !== $arrow_size ) {
			...
			et_pb_generate_responsive_css( $arrow_size_values, '%%order_class%% .swiper-button-prev, %%order_class%% .swiper-button-next', 'height', $render_slug );

			et_pb_generate_responsive_css( $arrow_size_values, '%%order_class%% .swiper-button-prev, %%order_class%% .swiper-button-next', 'width', $render_slug );
		...
		if ( '' !== $height_tablet || '' !== $height_phone || '' !== $height ) {
			...
			et_pb_generate_responsive_css( $height_values, '%%order_class%% .dsm_image_carousel_container', 'height', $render_slug );
		}*/
		...
		$this->add_classname(
			array(
				$arrow_position == 'inside' ? 'dsm_image_carousel_arrow_inside' : ' dsm_image_carousel_arrow_outside',
				$arrow_position_mobile == 'inside' ? 'dsm_image_carousel_arrow_mobile_inside' : ' dsm_image_carousel_arrow_mobile_outside',
				$infinite_smooth_scrolling === 'on' ? 'dsm_image_carousel_infinite_scroll' : '',
			)
		);
		...
		$data_attr[] = array(
			...
			'lightbox_gallery'               => $show_lightbox_gallery !== 'off' ? 'true' : 'false',
			'lightbox_caption'               => $show_lightbox_caption !== 'off' ? 'true' : 'false',
			'infinite_scrolling'             => 'on' === $infinite_smooth_scrolling ? 'true' : 'false',
		);

		$output = sprintf(
			'<div class="swiper-container dsm_image_carousel_container" data-params=%2$s><div class="swiper-wrapper">%1$s',
		...
		foreach ( $attachments as $id => $attachment ) {
			...
			if ( 'on' == $show_lightbox ) {
				$image_output = sprintf(
					'<a class="%4$s" href="%1$s" data-mfp-src="%1$s"><img src="%1$s" title="%2$s" alt="%2$s" data-dsm-image-description="%3$s" /></a>',
					esc_url( $attachment->image_src_full[0] ),
					esc_attr( $attachment->post_title ),
					esc_attr( $attachment->post_excerpt ),
					$show_lightbox == 'on' ? 'dsm_image_carousel_lightbox' : ''
				);
			} elseif ( '' !== $dsm_upload_gallery_custom_link_url ) {
				$image_output = sprintf(
					'<a href="%3$s" target="%4$s" %5$s><img src="%1$s" alt="%2$s" /></a>',
					esc_url( $attachment->image_src_full[0] ),
					esc_attr( $attachment->post_title ),
					esc_url( $dsm_upload_gallery_custom_link_url ),
					esc_attr( $dsm_upload_gallery_link_url_target ),
					( $dsm_upload_gallery_link_as_download_file === '1' ? ' download' : '' )
				);
			} else {
				$image_output = sprintf(
					'<img src="%1$s" alt="%2$s" />',
		...
		$output .= '</div>';
		$output .= '</div>';
		'off' !== $arrows && $infinite_smooth_scrolling === 'off' ? $output .= '<div class="swiper-button-prev"></div><div class="swiper-button-next"></div>' : '';
		'off' !== $dots && $infinite_smooth_scrolling === 'off' ? $output   .= '<div class="swiper-pagination"></div>' : '';
