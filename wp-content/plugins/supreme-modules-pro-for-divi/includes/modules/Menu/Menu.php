...
class DSM_NavMenu extends ET_Builder_Module {
	...
	public function get_fields() {
		...
		return array(
			...
			'menu_link_text_active_color'  => array(
				'label'        => esc_html__( 'Menu Link Active Color', 'dsm-supreme-modules-pro-for-divi' ),
				'type'         => 'color-alpha',
				'custom_color' => true,
				'tab_slug'     => 'advanced',
				'toggle_slug'  => 'menu_style',
	...
	static function get_dsm_navmenu( $args = array() ) {
		...
		$menuClass .= sprintf( ' %s', esc_attr( 'dsm-menu-style-type-' . $args['list_style_type'] ) );
		$menuClass .= sprintf( ' %s', esc_attr( 'dsm-menu-layout-' . $args['menu_layout'] ) );
		$menuClass .= ( 'on' === $args['menu_animation'] ? sprintf( ' %s', esc_attr( 'dsm-menu-animation dsm-menu-animation-' . $args['underline_animation'] ) ) : '' );
		$primaryNav = '';
