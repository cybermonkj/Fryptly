...
if ( ! class_exists( 'DSM_Settings' ) ) :
	class DSM_Settings {
		...
		function __construct() {
			...
			if ( isset( $_GET['page'] ) && ( $_GET['page'] == 'dsm_license_page' ) ) {
		...
		function admin_menu() {
			...
			if ( $dsm_plugin_menu_icon !== '' ) {
				...
				//$dsm_plugin_menu_icon = plugins_url( 'supreme-modules-pro-for-divi/admin/img/icon-128x128.svg' );
			...
			if ( $this->settings_api->get_option( 'dsm_use_header_footer', 'dsm_general' ) == 'on' ) {
				add_submenu_page( 'divi_supreme_settings', __( 'Divi Templates', 'dsm-supreme-modules-pro-for-divi' ), __( 'Divi Templates', 'dsm-supreme-modules-pro-for-divi' ), 'manage_options', 'edit.php?post_type=dsm_header_footer' );
			}*/
			if ( ! $this->licence->licence_key_verify() ) {
				$hookID = add_submenu_page( 'divi_supreme_settings', __( 'License', 'dsm-supreme-modules-pro-for-divi' ), __( 'License', 'dsm-supreme-modules-pro-for-divi' ), 'manage_options', 'dsm_license_page', array( $this, 'licence_form' ) );
			} else {
				$hookID = add_submenu_page( 'divi_supreme_settings', __( 'License', 'dsm-supreme-modules-pro-for-divi' ), __( 'License', 'dsm-supreme-modules-pro-for-divi' ), 'manage_options', 'dsm_license_page', array( $this, 'licence_deactivate_form' ) );
			}
			...
			$settings_fields = array(
				'dsm_general'               => array(
					...
					array(
						...
						'class'   => 'dsm-settings-checkbox',
						'label'   => __( 'Enable Divi Template', 'dsm-supreme-modules-pro-for-divi' ),
						'type'    => 'checkbox',
						'default' => 'off',
					),*/
					array(
				...
				'dsm_settings_misc'         => array(
					...
					'dsm_plugin_menu_icon'       => array(
						...
						'desc'              => sprintf(
							__( 'Image URL or <a href="%s" target="_blank">Dashicon class name</a> to use for icon. Custom image should be 20px by 20px.', 'dsm-supreme-modules-pro-for-divi' ),
		...
		function options_update() {
			if ( isset( $_POST['dsm_pro_license_form_submit'] ) ) {
		...
		function admin_notices() {
			...
			if ( count( $messages ) > 0 ) {
				echo "<div class='notice-warning notice fade'><p>" . implode( '</p><p>', $messages ) . '</p></div>';
			}
		}
			...
			if(is_multisite())
				{
					if(isset($screen->id) && $screen->id    ==  'settings_page_woo-ms-options-network')
						return;
					?><div data-dismissible="disable-done-notice-forever" class="notice notice-info is-dismissible"><p><?php _e( "Divi Supreme Pro: Please enter your", 'wooslt' ) ?> <a href="<?php echo network_admin_url() ?>settings.php?page=dsm_license_page_network"><?php _e( "Licence key", 'dsm-supreme-modules-pro-for-divi' ) ?></a> to get regular update.</p></div><?php
				}
				else {
			*/
			if ( isset( $screen->id ) && $screen->id == 'settings_page_dsm_license_page' ) {
				return;
			}
			?><div data-dismissible="disable-done-notice-forever" class="notice notice-info is-dismissible"><p><?php _e( 'Divi Supreme Pro: Please enter your', 'dsm-supreme-modules-pro-for-divi' ); ?> <a href="admin.php?page=dsm_license_page"><?php _e( 'Licence key', 'dsm-supreme-modules-pro-for-divi' ); ?></a> to get regular update.</p></div>
					<?php
					/*}*/
		}
		...
		function licence_form_submit() {
			...
			if ( isset( $_POST['dsm_pro_license_form_submit'] ) && isset( $_POST['dsm_pro_licence_deactivate'] ) && wp_verify_nonce( $_POST['dsm_pro_license'], 'dsm_pro_license' ) ) {
				global $slt_form_submit_messages;
				...
				if ( is_wp_error( $data ) || $data['response']['code'] != 200 ) {
					$slt_form_submit_messages[] .= __( 'There was a problem connecting to ', 'dsm-supreme-modules-pro-for-divi' ) . DSM_PRO_APP_API_URL;
					return;
				}
				...
				if ( isset( $response_block->status ) ) {
					if ( $response_block->status == 'success' && $response_block->status_code == 's201' ) {
						...
						if ( $response_block->status_code == 'e002' || $response_block->status_code == 'e104' ) {
							...
							$slt_form_submit_messages[] = __( 'There was a problem deactivating the licence: ', 'dsm-supreme-modules-pro-for-divi' ) . $response_block->message;

							return;
						}
					}
				} else {
					$slt_form_submit_messages[] = __( 'There was a problem with the data block received from ' . DSM_PRO_APP_API_URL, 'dsm-supreme-modules-pro-for-divi' );
					return;
				}
			...
			if ( isset( $_POST['dsm_pro_license_form_submit'] ) && wp_verify_nonce( $_POST['dsm_pro_license'], 'dsm_pro_license' ) ) {

				$license_key = isset( $_POST['license_key'] ) ? sanitize_key( trim( $_POST['license_key'] ) ) : '';

				if ( $license_key == '' ) {
						$slt_form_submit_messages[] = __( "Licence key can't be empty", 'dsm-supreme-modules-pro-for-divi' );
						return;
				}
				...
				if ( isset( $response_block->status ) ) {
					...
					if ( $response_block->status == 'success' && $response_block->status_code == 's100' ) {
						...
						$slt_form_submit_messages[] = __( 'There was a problem activating the licence: ', 'dsm-supreme-modules-pro-for-divi' ) . $response_block->message;
						return;
					}
				} else {
					$slt_form_submit_messages[] = __( 'There was a problem with the data block received from ' . DSM_PRO_APP_API_URL, 'dsm-supreme-modules-pro-for-divi' );
				...
				<div class="wrap">
					...
					<form id="form_data" name="form" method="post">
						<div class="postbox" style="padding: 0px 25px 25px; margin-top: 10px;">
							...
							<div class="explain"><?php _e( 'If you forgotten/lost your the license key, you can always retrieve it from your ', 'dsm-supreme-modules-pro-for-divi' ); ?> <a href="https://divisupreme.com/my-account/" target="_blank"><?php _e( 'Divi Supreme Account', 'dsm-supreme-modules-pro-for-divi' ); ?></a>. <?php _e( 'More license keys can be generate from your account.', 'dsm-supreme-modules-pro-for-divi' ); ?></a> 
							</div>
						</div>                        
						<p class="submit">
							<input type="submit" name="Submit" class="button-primary dsm-admin-button" value="<?php _e( 'Activate', 'dsm-supreme-modules-pro-for-divi' ); ?>">
						</p>
					</form> 
				</div> 
			<?php
		...
		function licence_deactivate_form() {
			...
			*/
			?>
			<div class="notice notice-success"><p><?php _e( 'Your Divi Supreme Pro license has been activated.', 'dsm-supreme-modules-pro-for-divi' ); ?></p></div>
			<div class="wrap"> 
				<h2><?php _e( 'Divi Supreme Pro License', 'dsm-supreme-modules-pro-for-divi' ); ?></h2>
				<form id="form_data" name="form" method="post">    
							...
							<tr>
								...
								<td><?php if ( $this->licence->is_local_instance() ) { ?>
									...
									<?php
										...
										?>
									<b><?php echo substr( $license_data['key'], 0, 20 ); ?>-xxxxxxxx-xxxxxxxx</b>
									<?php } ?></td>
							</tr>
						</tbody>
					</table>
						<div class="explain"><?php _e( 'You can generate more keys from your ', 'dsm-supreme-modules-pro-for-divi' ); ?> <a href="https://divisupreme.com/my-account/" target="_blank">Divi Supreme Account</a> 
						</div>
					</div>
