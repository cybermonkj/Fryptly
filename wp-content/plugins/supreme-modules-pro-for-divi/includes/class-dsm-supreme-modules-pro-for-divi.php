...
class Dsm_Supreme_Modules_Pro_For_Divi {
	...
	private function define_admin_hooks() {
		...
		// SVG Handler.
		if ( $this->settings_api->get_option( 'dsm_allow_mime_svg_upload', 'dsm_settings_misc' ) === 'on' ) {
			add_filter( 'upload_mimes', array( $this, 'dsm_upload_mime_types' ) );
		}
		...
		if ( $this->settings_api->get_option( 'dsm_use_header_footer', 'dsm_general' ) === 'on' ) {
			...
			add_action( 'wp_print_scripts', array( $this, 'dsm_custom_top_header' ), 25 );
			add_filter( 'body_class', array( $this, 'dsm_header_footer_top_header' ) );
			add_action( 'et_html_main_header', array( $this, 'dsm_custom_header' ) );
			add_filter( 'manage_edit-dsm_header_footer_columns', array( $this, 'dsm_divi_template_post_columns_header' ) );
			add_action( 'manage_dsm_header_footer_posts_custom_column', array( $this, 'dsm_divi_template_post_columns_content' ) );
		}
		*/
		...
		// Popup.
		if ( $this->settings_api->get_option( 'dsm_use_supreme_popup', 'dsm_general' ) === 'on' ) {
			$this->dsm_add_popup_modules();
			add_filter( 'et_module_shortcode_output', array( $this, 'output_modules_popup' ), 10, 3 );
		}

		// Read More.
		if ( $this->settings_api->get_option( 'dsm_use_readmore_content', 'dsm_general' ) === 'on' ) {
		...
		if ( $this->settings_api->get_option( 'dsm_use_library_widget', 'dsm_general' ) === 'on' ) {
			add_action(
				'widgets_init',
				function() {
	 ...
	 * @since 3.0.0
	 */
	public function dsm_upload_mime_types( $mimes ) {
		$new_mimes        = array();
	...
	public function dsm_add_action_plugin( $actions, $plugin_file ) {
		...
		if ( $plugin === $plugin_file ) {
			...
			$license  = array( 'license' => '<a href="' . esc_url( get_admin_url( null, 'admin.php?page=dsm_license_page' ) ) . '">' . __( 'License', 'ddsm-supreme-modules-pro-for-divi' ) . '</a>' );
	...
	public function dsm_admin_footer_text( $footer_text ) {
		...
		$is_divi_supreme_screen_license = ( $current_screen->id === 'divi-supreme-pro_page_dsm_license_page' );

		if ( $is_divi_supreme_screen || $is_divi_supreme_screen_footer || $is_divi_supreme_screen_license ) {
			$footer_text = sprintf(
	...
	public function dsm_header_footer_posttypes() {
		$labels = array(
			'name'               => esc_html__( 'Divi Templates', 'dsm-supreme-modules-pro-for-divi' ),
			'singular_name'      => esc_html__( 'Divi Template', 'dsm-supreme-modules-pro-for-divi' ),
			'add_new'            => esc_html__( 'Add New', 'dsm-supreme-modules-pro-for-divi' ),
			'add_new_item'       => esc_html__( 'Add New Template', 'dsm-supreme-modules-pro-for-divi' ),
			'edit_item'          => esc_html__( 'Edit Template', 'dsm-supreme-modules-pro-for-divi' ),
			'new_item'           => esc_html__( 'New Template', 'dsm-supreme-modules-pro-for-divi' ),
			'all_items'          => esc_html__( 'All Templates', 'dsm-supreme-modules-pro-for-divi' ),
			'view_item'          => esc_html__( 'View Template', 'dsm-supreme-modules-pro-for-divi' ),
			'search_items'       => esc_html__( 'Search Templates', 'dsm-supreme-modules-pro-for-divi' ),
			'not_found'          => esc_html__( 'Nothing found', 'dsm-supreme-modules-pro-for-divi' ),
			'not_found_in_trash' => esc_html__( 'Nothing found in Trash', 'dsm-supreme-modules-pro-for-divi' ),
			'parent_item_colon'  => '',
		);
	...
	public function dsm_header_footer_top_header( $classes ) {
		global $post;
		/*1.6.4 fix*/
		if ( ! $post ) {
			return false;
		}
		$template_post_custom = get_post_custom();

		if ( $post->post_type === 'dsm_header_footer' && $template_post_custom['dsm-header-footer-meta-box-options'][0] === 'top_header' ) {
			$classes[] = 'dsm-body-top-header';
		}
		return $classes;
	}
		...
		<div class="dsm-header-footer-meta-box dsm_<?php echo get_post_meta( $post->ID, 'dsm-header-footer-meta-box-options', true ); ?>">
			<p class="dsm-header-footer-meta-box-options">
				...
				<select name="dsm-header-footer-meta-box-options">
					<?php
						$option_values = array(
							'footer'           => __( 'Footer', 'dsm-supreme-modules-pro-for-divi' ),
							'404'              => __( '404', 'dsm-supreme-modules-pro-for-divi' ),
							'search_no_result' => __( 'Search No Result', 'dsm-supreme-modules-for-divi' ),
							'top_header'       => __( 'Top Header', 'dsm-supreme-modules-for-divi' ),
						);
			...
			<p class="dsm-conditional-meta-box-options">
				<label for="dsm-conditional-meta-box-options" style="display: block; font-weight: bold; margin-bottom: 5px;">Conditional:</label>
				<textarea name="dsm-conditional-meta-box-options" rows="5" style="width:100%;"><?php echo get_post_meta( $post->ID, 'dsm-conditional-meta-box-options', true ); ?></textarea>
			</p>
			...
			<p><?php _e( 'Note: Footer Template and Top Header will only show up on the frontend.', 'dsm-supreme-modules-pro-for-divi' ); ?></p>
		</div>
		<?php
	}
	public function dsm_add_header_footer_meta_box() {
		...
		add_meta_box('dsm_header_footer_setting_metabox', 'Divi Templates', array( $this, 'dsm_header_footer_setting_metabox' ), $post_types, 'side', 'high', null);
		*/
	...
	public function dsm_save_header_footer_meta_box( $post_id, $post, $update ) {
		...
		if ( isset( $_POST['dsm-conditional-meta-box-options'] ) ) {
			update_post_meta( $post_id, 'dsm-conditional-meta-box-options', sanitize_text_field( $_POST['dsm-conditional-meta-box-options'] ) );
		}
	...
	public function dsm_custom_footer() {
		...
		if ( $footer_template->have_posts() ) {
			...
			if ( $footer_template_css['dsm-conditional-meta-box-options'][0] !== '' ) {
				$footer_conditional_value = $footer_template_css['dsm-conditional-meta-box-options'][0];
			} else {
				$footer_conditional_value = '';
			}

			$footer_template_css_output = $footer_template_css['dsm-css-classes-meta-box-options'][0];

			$footer_condition         = "return $footer_conditional_value;";
			$footer_conditional_check = eval( $footer_condition );
	...
	public function dsm_header_footer_admin_notice( $post ) {
		...
		if ( $current_screen->post_type === 'dsm_header_footer' ) {
			...
			<div class="notice notice-info">
				<p><?php _e( 'Notice: For first time user, please re-save your <a href="' . get_admin_url() . 'options-permalink.php"  target="_blank">Permalinks</a> again to flush the rewrite rules in order view them in Visual Builder. This will only work for the Divi Theme. Once ElegantThemes updated their Template Hook on Extra Theme, this feature will also be available. Currently only the top header, footer, search no results and 404 template is available you. Please create one template and assign to the top header, footer or 404. If you do not see Divi Builder here, remember to <a href="' . get_admin_url() . 'admin.php?page=et_divi_options#wrap-builder" target="_blank">Enable Divi Builder On Post Types</a> in the Divi Options.', 'dsm-supreme-modules-pro-for-divi' ); ?></p>
			</div>
			<?php
		}
	}
	public function dsm_custom_top_header( $top_header ) {
		$dsm_top_header_args = array(
			'post_type'  => 'dsm_header_footer',
			'meta_key'   => 'dsm-header-footer-meta-box-options',
			'meta_value' => 'top_header',
			'meta_type'  => 'post',
			'meta_query' => array(
				array(
					'key'     => 'dsm-header-footer-meta-box-options',
		...
		$dsm_top_header_template = new WP_Query(
			$dsm_top_header_args
		);

		if ( $dsm_top_header_template->have_posts() ) {
			echo '<style id="dsm-top-header-css" type="text/css">.dsm-top-header { color: #666 !important; line-height: 1.7em !important; font-size: 14px  !important; }</style>';
		}
	}
	public function dsm_custom_header( $main_header ) {
		$dsm_top_header_args = array(
			'post_type'  => 'dsm_header_footer',
			'meta_key'   => 'dsm-header-footer-meta-box-options',
			'meta_value' => 'top_header',
		...
		if ( $dsm_top_header_template->have_posts() ) {
			$top_header_template_ID          = $dsm_top_header_template->post->ID;
			$top_header_template_shortcode   = do_shortcode( get_post_field( 'post_content', $top_header_template_ID ) );
			$top_header_template_post_custom = get_post_custom( $top_header_template_ID );

			if ( $top_header_template_post_custom['dsm-conditional-meta-box-options'][0] !== '' ) {
				$footer_conditional_value = $top_header_template_post_custom['dsm-conditional-meta-box-options'][0];
			} else {
				$footer_conditional_value = '';
			}

			$top_header_template_css_output = $top_header_template_post_custom['dsm-css-classes-meta-box-options'][0];

			$top_header_condition         = "return $footer_conditional_value;";
			$top_header_conditional_check = eval( $top_header_condition );

			$top_header_output = '';
			if ( ! et_core_is_fb_enabled() ) {
				if ( $top_header_template_post_custom['dsm-conditional-meta-box-options'][0] !== '' ) {
					if ( $top_header_conditional_check ) {
						$top_header_output = sprintf(
							'<div id="top-header" class="dsm-top-header%1$s">%2$s</div>
                            ',
							$top_header_template_css_output !== '' ? esc_attr( " $top_header_template_css_output" ) : '',
							$top_header_template_shortcode
						);
					}
				} else {
					$top_header_output = sprintf(
				...
				$top_header_output .= $main_header;
			} else {
				$top_header_output .= $main_header;
			}
			echo $top_header_output;
		} else {
			echo $main_header;
		}
	}
	public function dsm_divi_template_post_columns_header( $columns ) {
		$columns['template'] = __( 'Template', 'dsm-supreme-modules-pro-for-divi' );
		return $columns;
	}
	public function dsm_divi_template_post_columns_content( $column_name ) {
		global $post;

		switch ( $column_name ) {
			case 'template':
				$template = get_post_custom();
				$string   = str_replace( '_', ' ', $template['dsm-header-footer-meta-box-options'][0] );
				echo ucwords( $string );
				break;
		}
	}
	...
	public function output_modules_readmore( $output, $render_slug, $module ) {
		if ( dsm_load_readme_divi_modules() !== $render_slug ) {
			...
			if ( isset( $module->props['dsm_modules_readmore'] ) && $module->props['dsm_modules_readmore'] === 'on' ) {
				...
				if ( $module->props['dsm_modules_readmore_shadow'] === 'on' ) {
	...
	public function output_modules_popup( $output, $render_slug, $module ) {
		if ( $this->dsm_load_divi_modules() !== $render_slug ) {
			...
			if ( isset( $module->props['dsm_modules_popup'] ) && $module->props['dsm_modules_popup'] === 'on' ) {
				...
				$dsm_modules_popup_auto_close             = $module->props['dsm_modules_popup_auto_close'] === 'on' ? ' data-dsm-popup-auto-close-time="' . esc_attr( $module->props['dsm_modules_popup_auto_close_time'] ) . '"' : '';
				...
				$dsm_modules_popup_image_src              = $module->props['dsm_modules_popup_type'] === 'image' ? ' data-dsm-popup-image-src="' . esc_url( $module->props['dsm_modules_popup_image_src'] ) . '"' : '';
				$dsm_modules_popup_video_url              = $module->props['dsm_modules_popup_type'] === 'video' ? ' data-dsm-popup-video-url="' . esc_url( $module->props['dsm_modules_popup_video_url'] ) . '"' : '';
				$dsm_modules_popup_timed_delay            = $module->props['dsm_modules_popup_trigger'] === 'onload' || $module->props['dsm_modules_popup_trigger'] === 'scroll' || $module->props['dsm_modules_popup_trigger'] === 'exit' ? ' data-dsm-popup-timed-delay="' . floatval( $module->props['dsm_modules_popup_timed_delay'] ) * 1000 . '"' : '';
				$dsm_modules_popup_cookie_expiry          = $module->props['dsm_modules_popup_trigger'] === 'onload' || $module->props['dsm_modules_popup_trigger'] === 'scroll' ? ' data-dsm-popup-cookie-expiry="' . $module->props['dsm_modules_popup_cookie_days'] . '"' : '';
				...
				$dsm_modules_popup_check_trigger = $dsm_modules_popup_cookie_consent === 'on' ? '' : ' dsm-popup-trigger';
				$dsm_modules_popup_check_overlay = $dsm_modules_overlay !== 'off' ? 'data-dsm-popup-background="' . esc_html( $dsm_modules_overlay_background ) . '"' : '';
				...
				if ( $dsm_modules_popup_close_button_placement === 'false' ) {
					...
					if ( $dsm_modules_popup_type !== 'inline' ) {
						ET_Builder_Element::set_style(
							$render_slug,
							array(
								'selector'    => $dsm_popup_final_selector . ' .mfp-close',
				...
				if ( $dsm_modules_popup_fullwidth === 'off' ) {
	...
	public function dsm_add_scheduled_content_modules() {
		$dsm_load_divi_modules = $this->dsm_load_divi_modules();
	...
	public function output_section( $output, $render_slug, $module ) {
		if ( 'et_pb_section' !== $render_slug ) {
			...
			if ( isset( $module->props['dsm_section_schedule_visibility'] ) && $module->props['dsm_section_schedule_visibility'] === 'on' ) {
				...
				if ( $dsm_section_schedule_user_logged_in === 'logged-in' ) {
					...
					if ( ! empty( $dsm_section_schedule_user_roles[0] ) ) {
						...
						foreach ( array_keys( $combine, 'off' ) as $key ) {
							unset( $combine[ $key ] );
						}
				...
				if ( $dsm_section_schedule_day_week_on_off === 'on' ) {
					if ( ! empty( $dsm_section_schedule_day_week ) ) {
						...
						$current_day  = wp_date( 'N', null );
						$current_time = wp_date( 'H:i', null );
						$opening_time = date( 'H:i', strtotime( $dsm_section_schedule_opening_time ) );
						$closing_time = date( 'H:i', strtotime( $dsm_section_schedule_closing_time ) );

						foreach ( array_keys( $combine_days, 'off' ) as $key ) {
				...
				if ( $dsm_section_schedule_show_hide === 'start' ) {
					if ( $dsm_section_schedule_day_week_on_off === 'on' ) {
						if ( in_array( $current_day, $DSM_scheduled_days ) ) {
							if ( $current_time >= $opening_time && $current_time <= $closing_time ) {
								if ( $dsm_section_schedule_user_logged_in === 'visitors' ) {
									if ( ! is_user_logged_in() ) {
										$output;
									} else {
										return;
									}
								} elseif ( $dsm_section_schedule_user_logged_in === 'logged-in' ) {
						...
						if ( $dsm_section_schedule_between === 'on' ) {
	...
	public function output_row( $output, $render_slug, $module ) {
		if ( 'et_pb_row' !== $render_slug ) {
			...
			if ( isset( $module->props['dsm_row_schedule_visibility'] ) && $module->props['dsm_row_schedule_visibility'] === 'on' ) {
				...
				if ( $dsm_row_schedule_user_logged_in === 'logged-in' ) {
					$DSM_WP_user = wp_get_current_user();
				...
				if ( $dsm_row_schedule_day_week_on_off === 'on' ) {
					if ( ! empty( $dsm_row_schedule_day_week ) ) {
						...
						$current_day  = wp_date( 'N', null );
						$current_time = wp_date( 'H:i', null );
						$opening_time = date( 'H:i', strtotime( $dsm_row_schedule_opening_time ) );
						$closing_time = date( 'H:i', strtotime( $dsm_row_schedule_closing_time ) );
				...
				if ( $dsm_row_schedule_show_hide === 'start' ) {
					if ( $dsm_row_schedule_day_week_on_off === 'on' ) {
						if ( in_array( $current_day, $DSM_scheduled_days ) ) {
							if ( $current_time >= $opening_time && $current_time <= $closing_time ) {
								if ( $dsm_row_schedule_user_logged_in === 'visitors' ) {
									if ( ! is_user_logged_in() ) {
										$output;
									} else {
										return;
									}
								} elseif ( $dsm_row_schedule_user_logged_in === 'logged-in' ) {
									if ( ! isset( $DSM_allowed_user_roles ) ) {
										return;
									}
									if ( is_user_logged_in() && array_intersect( $DSM_allowed_user_roles, $DSM_WP_user->roles ) ) {
						...
						if ( $dsm_row_schedule_between === 'on' ) {
	...
	public function output_column( $output, $render_slug, $module ) {
		if ( 'et_pb_column' !== $render_slug ) {
			...
			if ( isset( $module->props['dsm_column_schedule_visibility'] ) && $module->props['dsm_column_schedule_visibility'] === 'on' ) {
				if ( is_array( $output ) ) {
					return $output;
				}
				...
				if ( $dsm_column_schedule_user_logged_in === 'logged-in' ) {
					$DSM_WP_user = wp_get_current_user();
				...
				if ( $dsm_column_schedule_day_week_on_off === 'on' ) {
					if ( ! empty( $dsm_column_schedule_day_week ) ) {
						...
						$current_day  = wp_date( 'N', null );
						$current_time = wp_date( 'H:i', null );
						$opening_time = date( 'H:i', strtotime( $dsm_column_schedule_opening_time ) );
						$closing_time = date( 'H:i', strtotime( $dsm_column_schedule_closing_time ) );
				...
				if ( $dsm_column_schedule_show_hide === 'start' ) {
					if ( $dsm_column_schedule_day_week_on_off === 'on' ) {
						if ( in_array( $current_day, $DSM_scheduled_days ) ) {
							if ( $current_time >= $opening_time && $current_time <= $closing_time ) {
								if ( $dsm_column_schedule_user_logged_in === 'visitors' ) {
									if ( ! is_user_logged_in() ) {
										$output;
									} else {
										return;
									}
								} elseif ( $dsm_column_schedule_user_logged_in === 'logged-in' ) {
									if ( is_user_logged_in() && array_intersect( $DSM_allowed_user_roles, $DSM_WP_user->roles ) ) {
										$output;
									} else {
										return;
						...
						if ( $dsm_column_schedule_between === 'on' ) {
	...
	public function output_modules( $output, $render_slug, $module ) {
		$dsm_divi_modules_all     = $this->dsm_load_divi_modules();
		$dsm_divi_modules_all     = array_combine( $dsm_divi_modules_all, $dsm_divi_modules_all );
		$dsm_divi_modules_defined = array_search( $render_slug, $dsm_divi_modules_all );

		if ( $dsm_divi_modules_defined === $render_slug ) {
			if ( isset( $module->props['dsm_modules_schedule_visibility'] ) && $module->props['dsm_modules_schedule_visibility'] === 'on' ) {
				if ( is_array( $output ) ) {
					return $output;
				}
				...
				if ( $dsm_modules_schedule_user_logged_in === 'logged-in' ) {
					$DSM_WP_user = wp_get_current_user();
				...
				if ( $dsm_modules_schedule_day_week_on_off === 'on' ) {
					if ( ! empty( $dsm_modules_schedule_day_week ) ) {
						...
						$current_day  = wp_date( 'N', null );
						$current_time = wp_date( 'H:i', null );
						$opening_time = date( 'H:i', strtotime( $dsm_modules_schedule_opening_time ) );
						$closing_time = date( 'H:i', strtotime( $dsm_modules_schedule_closing_time ) );
				...
				if ( $dsm_modules_schedule_show_hide === 'start' ) {
					if ( $dsm_modules_schedule_day_week_on_off === 'on' ) {
						if ( in_array( $current_day, $DSM_scheduled_days ) ) {
							if ( $current_time >= $opening_time && $current_time <= $closing_time ) {
								if ( $dsm_modules_schedule_user_logged_in === 'visitors' ) {
									if ( ! is_user_logged_in() ) {
										$output;
									} else {
										return;
									}
								} elseif ( $dsm_modules_schedule_user_logged_in === 'logged-in' ) {
									if ( ! isset( $DSM_allowed_user_roles ) ) {
										return;
									}
									if ( is_user_logged_in() && array_intersect( $DSM_allowed_user_roles, $DSM_WP_user->roles ) ) {
						...
						if ( $dsm_modules_schedule_between === 'on' ) {
