...
class Dsm_Supreme_Modules_Pro_For_Divi_Public {
	...
	public function enqueue_styles() {
		...
		if ( $easy_theme_builder === true ) {
			...
			if ( '' !== $easy_theme_builder_menu ) {
				$dsm_tb_header_css .= '.dsm_fixed_header_shrink.dsm_fixed_header_shrink_active header.et-l--header .et_pb_module.et_pb_menu { padding-top: ' . floatval( $easy_theme_builder_section ) . 'px; padding-bottom: ' . floatval( $easy_theme_builder_section ) . 'px; } ';
			}
	...
	public function enqueue_scripts() {
		...
		if ( isset( get_option( 'dsm_general' )['dsm_use_builder_responsive_viewer'] ) && 'off' !== get_option( 'dsm_general' )['dsm_use_builder_responsive_viewer'] ) {
			if ( function_exists( 'et_core_is_fb_enabled' ) && function_exists( 'et_builder_bfb_enabled' ) ) {
				if ( et_core_is_fb_enabled() || et_builder_bfb_enabled() ) {
					wp_enqueue_script( 'divi-responsive-viewer-fb', plugin_dir_url( __DIR__ ) . 'admin/js/dsm-responsive-builder.js', array( 'et-frontend-builder' ), DSM_PRO_VERSION, false );
				}
			}
			if ( function_exists( 'et_builder_tb_enabled' ) ) {
				if ( et_builder_tb_enabled() ) {
					wp_dequeue_script( 'divi-responsive-viewer-fb' );
					wp_enqueue_script( 'divi-responsive-viewer-tb', plugin_dir_url( __DIR__ ) . 'admin/js/dsm-responsive-builder-tb.js', array( 'et-frontend-builder' ), DSM_PRO_VERSION, false );
				}
			}
		}
		...
		wp_register_script( 'dsm-swiper', plugin_dir_url( __FILE__ ) . 'js/swiper.min.js', array(), DSM_PRO_VERSION, true );
		...
		wp_register_script( 'dsm-popup', plugin_dir_url( __DIR__ ) . 'includes/extensions/Popup/frontend.min.js', array( 'jquery' ), DSM_PRO_VERSION, true );
	}
