<?php if (empty($attr['uap_no_fb_js'])):?>
<SCRIPT>
	///////////////////FACEBOOK
...
<?php
	...
	if (!empty($attr['is_affiliates'])){
		/// AFFILIATES
		if (!empty($attr['custom_description'])){
			$attr['description'] = ism_format_like_post_description( stripslashes($attr['custom_description']));
		}
		if (!empty($attr['custom_url'])){
			$attr['url'] = $attr['custom_url'];
		}
	}
