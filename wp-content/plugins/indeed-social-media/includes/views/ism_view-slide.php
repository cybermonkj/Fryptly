<?php
	...
	/********************* HTML **************************/
    $html = '<div class="ism_slide_box" id="'.$attr['parent_before_wrap_id'].'">'
				. '<div class="ism_top_side_slide">' . $meta_arr['title'] . '</div>'
				. '<div class="ism_close_slide" onClick="ism_close_slide(\'#'.$attr['parent_before_wrap_id'].'\');"></div>'
				. '<div class="ism_popup_above_content_slide">'.ims_format_source_text(stripslashes($meta_arr['above_content'])).'</div>'
				. '<div class="ism_wrap '.$meta_arr['template'].' '.$meta_arr['type'].'" id="'.$attr['indeed_wrap_id'].'">' 
					. $html 
				. '</div>'
				. '<div class="ism_popup_bellow_content_slide">'.ims_format_source_text(stripslashes($meta_arr['bellow_content'])).'</div>'
			  . '</div>';
    $html .= ismAfterShareHtml($meta_arr, $attr);
    
    /****************** CSS ******************/
    $css .= ism_style_for_slide($attr, $meta_arr);
    $css .= '
			'.$meta_arr['custom_css'];
	$css = "<style>" . $css . "</style>";
    ...
    $js .= ism_show_up_slide($attr['parent_before_wrap_id'], $meta_arr);
    $js = "<script>" . $js . "</script>";   
