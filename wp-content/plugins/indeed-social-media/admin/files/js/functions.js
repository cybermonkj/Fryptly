...
function ism_deleting_all_data() {
  swal({
   title: "Are you sure you want to erase the table?",
   text: "Once deleted, you will not be able to recover email addresses!",
   type: "warning",
...
function ism_export_email_list() {
  jQuery.ajax({
    type : "post",
    url : window.ism_base_path +'/wp-admin/admin-ajax.php',
    data : {
         action  : "ism_delete_emails_from_table",
...
// trigger wich will export csv file for subscribers
...
success: function (response) {
    ...
    if(response) {
        ...
        jQuery('#csv_file').attr('href', response).css('display', 'inline');
