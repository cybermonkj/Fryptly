...
class OnpLicensing325_LicenseManagerPage extends FactoryPages321_AdminPage  {
    ...
    public function indexAction() {
        ...
        <div class="factory-bootstrap-331 factory-fontawesome-320 onp-page-wrap <?php echo $licenseData['Category'] ?>-license-manager-content" id="license-manager">
            ...
            <?php } else { ?>
                <div class="license-message <?php echo $licenseData['Category'] ?>-license-message">
                    ...
                    <div class="alert alert-normal alert-warning-icon">
                        <strong><?php _e('The key has been deleted successfully.', 'onp_licensing_325') ?></strong>
                        <p><?php _e('Please check the <a href="plugins.php">Plugins</a> page and update the plugin to complete deletion if it\'s needed.', 'onp_licensing_325') ?></p>    
                    </div>
                    <?php } ?>     
                    
                    <?php if ($scope == 'reset-license') { ?>
                    <div class="alert alert-success">
                        <?php _e('Your license details have been reset successfully.', 'onp_licensing_325') ?>
                    </div>
                    <?php } ?>                      

                    <?php if ($scope == 'check-updates') { ?>
                    <div class="alert alert-normal">
                        <strong><?php _e('The updates have been checked successfully.', 'onp_licensing_325') ?></strong>
                        <p>
                        <?php if ( $updatesManager->isActualVersion() ) { ?>
                            <?php _e('You use the actual version of the plugin.', 'onp_licensing_325') ?>
                        <?php } else { ?>
                            <?php if ( $updatesManager->needChangeAssembly() ) { ?>
                            <?php printf( 
                                    __('You need to upgrade to the %1$s version. <a href="plugins.php">Click here</a> to get the update.', 'onp_licensing_325'), $this->plugin->license->build  ) ?>
                            <?php } else { ?>
                            <?php printf( 
                                    __('The %1$s version is available to download. <a href="plugins.php">Click here</a> to get the update.', 'onp_licensing_325'),
            ...
            <div class="onp-container">
                <div class="license-details">
                    <?php ?> 
                        <?php global $sociallocker;
if ( !in_array( $sociallocker->license->type, array( 'paid' ) ) ) {
 ?>
                        <a href="<?php onp_licensing_325_purchase_url( $this->plugin ) ?>" id="purchase-premium">
                            <span class="btn btn-gold btn-inner-wrap">
                            <?php if ( !empty( $this->purchasePrice ) ) { ?>
                            <i class="fa fa-star"></i> <?php printf( __('Get Key For %1$s Only', 'onp_licensing_325'), $this->purchasePrice ) ?> <i class="fa fa-star"></i></i>
                            <?php } else { ?>
                            <i class="fa fa-star"></i> <?php _e('Get Premium Key', 'onp_licensing_325') ?> <i class="fa fa-star"></i></i>
                            <?php } ?>
                            </span>
                        </a> 
                        <?php 
 ...
 ?>   
                    ...
                    <p><?php _e('Your current license:', 'onp_licensing_325') ?></p>
                    <?php } else { ?>
                    <p><?php printf( __('Your current license for %1$s:', 'onp_licensing_325'), $this->plugin->pluginTitle ) ?></p>   
                    <?php } ?>
                    <div class="license-details-block <?php echo $licenseData['Category'] ?>-details-block">
                        
                        <?php if ( $licenseManager->hasKey() ) { ?>
                        <a href="<?php $this->actionUrl('deleteKey') ?>" class="btn btn-default btn-small license-delete-button"><i class="icon-remove-sign"></i> <?php _e('Delete Key', 'onp_licensing_325') ?></a>
                        <?php } ?>
                        ...
                        <?php if ( $licenseManager->data['Build'] !== 'free' ) { ?>
                            <?php if ( $licenseManager->data['Category'] == 'free' ) { ?>
                                ...
                                <p>
                                    <?php _e('Please, activate the plugin to unlock its features. Enter the key you received with the plugin into the form below. Don\'t know where the key is? <a href="#" id="open-faq">Click here</a>.', 'onp_licensing_325') ?>
                                </p>
                                <?php } else { ?>
                                <p>
                                    <?php _e('Please, activate the plugin to unlock its features. Enter the key you received with the plugin into the form below.', 'onp_licensing_325') ?>
                                </p>
                                <?php } ?>
                                <?php if ( $this->trial ) { ?>
                                <p class="activate-trial-hint">
                                    <?php printf( __('Also you can activate a <a href="%1$s">trial version</a> for 7 days to test the plugin on this site.', 'onp_licensing_325'), $this->getActionUrl('activateTrial') ) ?>
                                </p>
                                <?php } ?>
                            <?php } else { ?>
                        ...
                        <table class="license-params" colspacing="0" colpadding="0">
                            <tr>
                                <td class="license-param license-param-domain">
                                    ...
                                    <span class="license-value-name"><?php _e('domain', 'onp_licensing_325') ?></span>
                                </td>   
                                <td class="license-param license-param-version">
                                    <span class="license-value"><?php echo $this->plugin->version ?> <small><?php _e('version', 'onp_licensing_325') ?></small></span>
                                    <?php if ( $updatesManager->isVersionChecked() ) { ?>
                                        <?php if ( $updatesManager->isActualVersion() ) { ?>
                                            <span class="license-value-name"><?php _e('up-to-date', 'onp_licensing_325') ?></span>
                                        <?php } else { ?>
                                            <?php if ( $updatesManager->needChangeAssembly() ) { ?>
                                            <span class="license-value-name">
                                                <a href="plugins.php" class="link-to-upgrade">
                                                <?php printf( __('upgrade to %s', 'onp_licensing_325'), $this->plugin->license->build ) ?>
                                                </a>
                                            </span>    
                                            <?php } else { ?>
                                            <span class="license-value-name">
                                                <a href="plugins.php" class="link-to-upgrade">
                                                <?php echo $updatesManager->lastCheck['Build'] ?>-<?php echo $updatesManager->lastCheck['Version'] ?> <?php _e('available', 'onp_licensing_325') ?>
                                                </a>
                                            </span>
                                            <?php } ?>
                                        <?php } ?>
                                    <?php } else { ?>
                                    <span class="license-value-name"><span><?php _e('up-to-date', 'onp_licensing_325') ?></span></span>
                                    <?php } ?>
                                </td>  
                                <td class="license-param license-param-days">
                                    <span class="license-value"><?php echo $licenseManager->data['Build'] ?></span>                                   
                                    <span class="license-value-name"><?php _e('assembly', 'onp_licensing_325') ?></span>
                                </td>
                                <td class="license-param license-param-days">
                                    <?php if ( $licenseManager->isExpired() ) {?>
                                        <span class="license-value"><?php _e('EXPIRED!', 'onp_licensing_325') ?></span>
                                        <span class="license-value-name"><?php _e('please update the key', 'onp_licensing_325') ?></span>
                                    <?php } else { ?>  
                                        <span class="license-value">                               
                                            <?php if ( $isInfinity ) {?><?php _e('infinity', 'onp_licensing_325') ?><?php } else { ?>
                                                <?php echo $remained; ?><small> <?php _e('day(s)', 'onp_licensing_325') ?></small>
                                            <?php } ?>
                                        </span>
                                        <span class="license-value-name"><?php _e('remained', 'onp_licensing_325') ?></span>
                                    <?php } ?>    
                                </td>
                            </tr>
                        </table>
                ...
                <div class="license-input">
                    <form action="<?php $this->actionUrl("index") ?>" method="post">
                        ...
                        <?php if ( $this->premium ) { ?>
                            <p><?php _e('Have a key to activate the premium version? Paste it here:', 'onp_licensing_325') ?><p>
                        <?php } else { ?>
                            <p><?php _e('Have a key to activate the plugin? Paste it here:', 'onp_licensing_325') ?><p>
                        <?php } ?>
                            
                        <a href="#" class="btn btn-default" id="license-submit">
                            <?php _e('Submit Key', 'onp_licensing_325') ?>
                        </a>  
                        ...
                        <?php if ( $this->premium ) { ?>
                            <p style="margin-top: 10px;">
                                <?php printf( __('<a href="%1$s">Lean more</a> about the premium version and get the license key to activate it now!', 'onp_licensing_325'), onp_licensing_325_get_purchase_url( $this->plugin ) ) ?>
                            </p>
                        <?php } else { ?>
                            <p style="margin-top: 10px;">
                                <?php printf( __('<a href="%1$s">Lean more</a> about this plugin and get the license key to activate it now!', 'onp_licensing_325'), onp_licensing_325_get_purchase_url( $this->plugin ) ) ?>
                            </p>
                        <?php } ?>
                    </form>
                </div>
            ...
            <div id="plugin-update-block">
                <?php if ( $updatesManager->needCheckUpdates() ) { ?>
                    <?php if ( !$updatesManager->isVersionChecked() ) { ?>
                        ...
                        <?php } else { ?>
                            <?php _e('The updates were checked <strong>never</strong>.', 'onp_licensing_325') ?>
                        <?php } ?>
                    <?php } else { ?>
                        <?php if ( $updatesManager->isActualVersion() ) { ?>
                            <?php printf( __('The updates were checked at <strong>%1$s</strong>, you use the up-to-date version. ', 'onp_licensing_325'), date( 'g:i a, j M y', $updatesManager->lastCheck['Checked'] ) ) ?>
                        <?php } else { ?>
                            <?php printf( __('The updates were checked at <strong>%1$s</strong>, <strong>%2$s</strong>. ', 'onp_licensing_325'), date( 'g:i a, j M y', $updatesManager->lastCheck['Checked'] ), $updatesManager->lastCheck['Version'] ) ?>
                        <?php } ?>
                    <?php } ?>
                    <?php printf( __('Click <a href="%1$s">here</a> to check updates manually.', 'onp_licensing_325'), $this->getActionUrl('checkUpdates') ) ?>
                <?php } ?>
                <span class="gray-link">[ <a href="<?php echo $this->getActionUrl('internalKeys') ?>"><?php _e('internal keys', 'onp_licensing_325') ?></a> ]</span>       
            </div>
                ...
                <ul>
                    <li>
                        <a class="faq-header" id="how-to-find-the-key">
                            <?php _e('I purchased the plugin on CodeCanyon, but I cannot find the license key. Where is it?', 'onp_licensing_325') ?>
                        </a>
                        <div>
                            <p>
                                <?php _e('If you purchased the plugin on CodeCanyon, visit your Downloads section and click Licence Certificate.', 'onp_licensing_325') ?>
                                <?php _e('Find Item Purchase Code in the text document and paste it into the form above.', 'onp_licensing_325') ?>
                            </p>
                            <p style="text-align: center;">
                                <img src="<?php echo ONP_LICENSING_325_URL . '/assets/img/how-to-find-key.png' ?>" />
                            </p>
                        </div>
                    </li>
                    <li>
                        <a class="faq-header">
                            <?php _e('The plugin I purchased comes with the bonus. Where I can find a key to activate the bonus?', 'onp_licensing_325') ?>
                        </a>
                        <div>
                            <p>
                                <?php _e('You can activate the bonus plugin via the key you got for the principal plugin.', 'onp_licensing_325') ?>
                            </p>
                        </div>
                    </li>
                    <li>
                        <a class="faq-header">
                            <?php _e('Is it possible to get the premium version without the License Manager?', 'onp_licensing_325') ?>
                        </a>
                        <div>
                            <p>
                                <?php _e('Yes, it\'s possible if you want to distribute the plugin as a part of your product. Please contact us to discuss details: support@onepress-media.com', 'onp_licensing_325') ?>
                            </p>
                        </div>
                    </li>
                    <li>
