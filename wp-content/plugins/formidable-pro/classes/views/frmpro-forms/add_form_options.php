...
<?php if ( $has_file_field && strpos( FrmAppHelper::get_server_value( 'SERVER_SOFTWARE' ), 'nginx' ) !== false ) { ?>
	<p class="howto">
		<?php esc_html_e( 'File protection is not currently supported on NGINX servers.', 'formidable-pro' ); ?>
		<input type="hidden" value="<?php echo esc_attr( $values['protect_files'] ); ?>" name="options[protect_files]" />
	</p>
<?php } else if ( $has_file_field ) { ?>
	<p>
