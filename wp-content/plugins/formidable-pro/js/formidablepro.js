function frmProFormJS() {
	...
	function decodeEntities( string ) {
		var p = document.createElement('p'),
			decoded;
		p.innerHTML = string;
		decoded = p.textContent || p.innerText;
		p = null;
		return decoded;
	}
	...
	function doSingleCalculation( allCalcs, fieldKey, vals, triggerField ) {
		...
		if ( ( isNumeric( dec ) || thisCalc.is_currency ) && totalField.attr( 'type' ) === 'number' ) {
				...
				if ( r.html ) {
					...
					jQuery( html ).find( '.wp-editor-area' ).each( function() {
						var key = Object.keys( tinyMCEPreInit.mceInit )[0],
							...
							newValues = {
								selector: '#' + this.id,
								body_class: orgSettings.body_class.replace( key, this.id )
							},
