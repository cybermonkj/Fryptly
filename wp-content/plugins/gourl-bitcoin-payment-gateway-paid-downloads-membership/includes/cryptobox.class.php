...
// end class
	...
	*/
	...
	/* C. Function payment_ipntest()
	*
	* Returns an array with test payments sent from the IPN TEST gourl.io web page -
	* https://gourl.io/info/ipn/IPN_Website_Testing.html
	*
	* It includes -
	* paymentID			- current record id in the table crypto_payments.
	* boxID				- your cryptobox id, the same as on gourl.io member page
	...
	* recordCreated		- GMT time a payment record was created in your database
	*/
	function payment_ipntest($boxID = "")
	{
		if ($boxID && (!is_numeric($boxID) || $boxID < 1 || round($boxID) != $boxID))	return false;

		$res = run_sql("SELECT paymentID, boxID, boxType, orderID, userID, countryID, coinLabel, amount, amountUSD, addr, txID, txDate, txConfirmed, processed, processedDate, recordCreated
						FROM crypto_payments WHERE ".($boxID?"boxID = ".intval($boxID)." && ":"")." orderID IN ('','demo_order1','demo_order2','demo_order3') and userID IN ('','test_user1','test_user2','test_user3') and addr LIKE 'TEST%' and txID LIKE 'TEST%' ORDER BY txDate DESC LIMIT 10000");
	...
	/* D. Function payment_ipntest_delete()
	*
	* Delete test payments sent from the IPN TEST gourl.io web page -
	* https://gourl.io/info/ipn/IPN_Website_Testing.html
	*/
	function payment_ipntest_delete($boxID = "")
	{
		if ($boxID && (!is_numeric($boxID) || $boxID < 1 || round($boxID) != $boxID))	return false;

		run_sql("DELETE FROM crypto_payments WHERE ".($boxID?"boxID = ".intval($boxID)." && ":"")." orderID IN ('','demo_order1','demo_order2','demo_order3') and userID IN ('','test_user1','test_user2','test_user3') and addr LIKE 'TEST%' and txID LIKE 'TEST%' LIMIT 10000");
	...
	/* E. Function cryptobox_sellanguage($default = "en")
	 *
	...
	/* G. Function display_language_box()
	 *
	...
	/* H. Function display_currency_box()
	 *
	...
	*/
	...
	/* J. Function convert_currency_live()
	 *
