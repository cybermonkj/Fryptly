...
$sb_apps = ['dialogflow', 'slack', 'wordpress', 'tickets', 'woocommerce'];
...
function sb_js_admin() {
    ...
    $code .= 'var SB_VERSIONS = { sb: "' . SB_VERSION . '", dialogflow: "' . (defined('SB_DIALOGFLOW') ? SB_DIALOGFLOW : -1) . '", slack: "' . (defined('SB_SLACK') ? SB_SLACK : -1) . '", tickets: "' . (defined('SB_TICKETS') ? SB_TICKETS : -1) . '"' . $wp_apps . '};';
    $code .= '</script>';
    echo $code;
}
 ...
 * -----------------------------------------------------------
 *
 * Add a new extra finromation for the user
 *
 */
...
function sb_new_conversation($user_id, $status_code = 0, $title = '', $department = -1, $agent_id = -1, $routing = false) {
    ...
    $conversation_id = sb_db_query('INSERT INTO sb_conversations(user_id, title, status_code, creation_time, department, agent_id) VALUES ("' . $user_id . '", "' . sb_db_escape(htmlspecialchars(ucfirst($title), ENT_NOQUOTES)) . '", "' . $status_code . '", "' . gmdate('Y-m-d H:i:s') . '", ' . (sb_isset_num($department) ? $department : 'NULL') . ', ' . (sb_isset_num($agent_id) ? $agent_id : 'NULL') . ')', true);
...
function sb_text_formatting_to_html($message) {
    $regex = [['/\*(.*?)\*/', '<b>', '</b>'], ['/_(.*?)_/', '<em>', '</em>'], ['/~(.*?)~/', '<del>', '</del>'], ['/```(.*?)```/', '<code>', '</code>'], ['/`(.*?)`/', '<code>', '</code>']];
