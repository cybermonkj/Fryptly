            ...
            if (action == 'all') {
                SBF.ajax({
                    ...
                    for (var i = 0; i < user_types.length; i++) {
                        this.updateMenuItem(menu, 'set', user_types[i], response[user_types[i]]);
                    }
                });
            } else {
                this.updateMenuItem(menu, action, type);
            }
        },
                            ...
                            if (!id_check.includes(response[i]['conversation_id'])) {
                                ...
                                if (!SBChat.tab_active && (!SBF.isAgent(item['message_user_type']) || item['message_user_type'] == 'bot')) {
                                    if (this.desktop_notifications) {
                                        SBChat.desktopNotification(item['first_name'] + ' ' + item['last_name'], item['message'], (item['profile_image'].indexOf('user.svg') > 0 ? SB_ADMIN_SETTINGS['notifications-icon'] : item['profile_image']), conversation_id);
                                    }
    ...
    $(document).ready(function () {
        ...
        $(header).on('click', '.sb-version', function () {
            ...
            SBF.ajax({
                ...
                let code = '';
                let names = { 'sb': 'Support Board', 'slack': 'Slack', 'dialogflow': 'Dialogflow', 'tickets': 'Tickets', 'woocommerce': 'Woocommerce' };
                let updates = false;
