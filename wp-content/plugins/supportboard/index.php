...
function sb_enqueue() {
    ...
    if (sb_get_setting('wp-manual') !== true) {
        ...
        $inline_code .= 'var SB_INIT_URL = "' . SB_URL . '/js/min/init.min.js' . $lang . '"; var SB_WP_ACTIVE_USER = ' . $user_data . '; var SB_WP_PAGE_ID = ' . ($page_id == '' ? -1 : $page_id) . '; var SB_WP_AJAX_URL = "' . admin_url('admin-ajax.php'). '"';
        ...
        // Load scripts
        wp_enqueue_script('sb-js', SB_URL . '/js/init.js' . $lang, ['jquery'], SB_VERSION);
...
function sb_init_wp_agent() {
    if (!sb_is_agent()) {
        ...
        if (sb_is_error($db_user)) die($db_user);
